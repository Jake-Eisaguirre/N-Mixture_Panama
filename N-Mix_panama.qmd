...
#for (i in 1:length(spec_list)){
   ...
   table <- nmix_clean %>% 
     ...
     pivot_wider(id_cols = c("site", "visit_group") ,names_from = c("n_visit"), values_from = c("n", "survey_time")) %>% 
   ...
   umf <- unmarkedFramePCount(y = table,
                              obsCovs = list(survey_time = surv_cov))
